...
void CQ::GroupMemberInfo::Void()
{
	...
	BadRecordMember = 0;
	UniqueRank = "";
	UniqueRankExpireTime = 0;
	AbleToModifyGroupNick = 0;
}
...
void CQ::GroupMemberInfo::setdata(Unpack& u)
{
	...
	GroupNickname = u.getstring();
	Gender = u.getInt();
	Age = u.getInt();
	Region = u.getstring();
	AddtoGroupTime = u.getInt();
	LastMsgTime = u.getInt();
	GradeName = u.getstring();
	permissions = u.getInt();
	BadRecordMember = u.getInt() == 1;
	UniqueRank = u.getstring();
	UniqueRankExpireTime = u.getInt();
	AbleToModifyGroupNick = u.getInt() == 1;
}
...
{
	string s = "{";
	s += "群号:"; s += to_string(GroupID);
	s += " ,QQID:"; s += to_string(QQID);
	s += " ,昵称:"; s += Nickname;
	s += " ,名片:"; s += GroupNickname;
	s += " ,性别:"; s += (Gender == 255 ? "未知" : Gender == 1 ? "男" : "女");
	s += " ,年龄:"; s += to_string(Age);
	s += " ,地区:"; s += Region;
	s += " ,加群时间:"; s += to_string(AddtoGroupTime);
	s += " ,最后发言:"; s += to_string(LastMsgTime);
	s += " ,等级_名称:"; s += GradeName;
	s += " ,管理权限:"; s += (permissions == 3 ? "群主" : permissions == 2 ? "管理员" : "群员"); s += "("; s += to_string(permissions); s += ")";
	s += " ,不良记录成员:"; s += to_string(BadRecordMember);
	s += " ,专属头衔:"; s += UniqueRank;
	s += " ,专属头衔过期时间:"; s += to_string(UniqueRankExpireTime);
	s += " ,允许修改名片:"; s += to_string(AbleToModifyGroupNick);
	s += "}"; return s;
}

//增加运行日志
int CQ::addLog(int Priority, const char *Type, const char *Content) { return lasterr = CQ_addLog(getAuthCode(), Priority, Type, Content); }
...
int CQ::sendDiscussMsg(long long DiscussGroupID, const char *msg) { return   CQ_sendDiscussMsg(getAuthCode(), DiscussGroupID, msg); }

//发送讨论组消息
int CQ::sendDiscussMsg(long long DiscussGroupID, std::string & msg) { return sendDiscussMsg(DiscussGroupID, msg.c_str()); }
...
//置群员移除
int CQ::setGroupKick(long long GroupID, long long QQID, CQBOOL Refuse_Request_Forever) { return lasterr = CQ_setGroupKick(getAuthCode(), GroupID, QQID, Refuse_Request_Forever); }
...
//置群管理员
int CQ::setGroupAdmin(long long GroupID, long long QQID, CQBOOL Administrator) { return lasterr = CQ_setGroupAdmin(getAuthCode(), GroupID, QQID, Administrator); }

//置群成员专属头衔
int CQ::setGroupSpecialTitle(long long GroupID, long long QQID, const char *Rank, long long ExpireTime) { return lasterr = CQ_setGroupSpecialTitle(getAuthCode(), GroupID, QQID, Rank, ExpireTime); }

//置群成员专属头衔
int CQ::setGroupSpecialTitle(long long GroupID, long long QQID, std::string & Rank, long long ExpireTime) { return setGroupSpecialTitle(GroupID, QQID, Rank.c_str(),ExpireTime); }

//置全群禁言
int CQ::setGroupWholeBan(long long GroupID, CQBOOL WholeBan) { return lasterr = CQ_setGroupWholeBan(getAuthCode(), GroupID, WholeBan); }
...
//置群匿名设置
int CQ::setGroupAnonymous(long long GroupID, CQBOOL Anonymous) { return lasterr = CQ_setGroupAnonymous(getAuthCode(), GroupID, Anonymous); }

//置群成员名片
int CQ::setGroupCard(long long GroupID, long long QQID, const char *NewGroupNickname) { return lasterr = CQ_setGroupCard(getAuthCode(), GroupID, QQID, NewGroupNickname); }

//置群成员名片
int CQ::setGroupCard(long long GroupID, long long QQID, std::string NewGroupNickname) { return setGroupCard(GroupID, QQID, NewGroupNickname.c_str()); }

//置群退出
int CQ::setGroupLeave(long long GroupID, CQBOOL Dismiss) { return lasterr = CQ_setGroupLeave(getAuthCode(), GroupID, Dismiss); }

//置讨论组退出
int CQ::setDiscussLeave(long long DiscussGroupID) { return lasterr = CQ_setDiscussLeave(getAuthCode(), DiscussGroupID); }

//置好友添加请求
int CQ::setFriendAddRequest(const char *ResponseFlag, int ResponseType, const char *Remark) { return lasterr = CQ_setFriendAddRequest(getAuthCode(), ResponseFlag, ResponseType, Remark); }

//置群添加请求
int CQ::setGroupAddRequest(const char *ResponseFlag, int RequestType , int ResponseType, const char *Reason) { return lasterr = CQ_setGroupAddRequestV2(getAuthCode(), ResponseFlag, RequestType, ResponseType, Reason); }

//置致命错误提示
int CQ::setFatal(const char *ErrMsg) { return lasterr = CQ_setFatal(getAuthCode(), ErrMsg); }
