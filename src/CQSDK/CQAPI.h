...
namespace CQ{
	...
	CQAPI(CQ_sendDiscussMsg, int)(
		int AuthCode,// 
		long long DiscussGroupID,// 目标讨论组 
		const char *msg// 消息内容 
		);
	//发送赞 Auth=110
	...
	CQAPI(CQ_setGroupKick, int)(
		...
		long long GroupID,// 目标群 
		long long QQID,// 目标QQ 
		CQBOOL Refuse_Apply_Forever// 如果为真，则“不再接收此人加群申请”，请慎用 
		);
	//置群员禁言 Auth=121 
	CQAPI(CQ_setGroupBan, int)(
		...
		long long GroupID,// 目标群 
		long long QQID,// 目标QQ 
		long long BanTime// 禁言的时间，单位为秒。如果要解禁，这里填写0 
		);
	//置群管理员 Auth=122 
	CQAPI(CQ_setGroupAdmin, int)(
		...
		CQBOOL Administrator// 真/设置管理员 假/取消管理员 
		);
	...
	CQAPI(CQ_setGroupAnonymousBan, int)(
		...
		long long BanTime// 禁言的时间，单位为秒。不支持解禁 
		);
	//置群匿名设置 Auth=125 
	...
	CQAPI(CQ_setGroupCard, int)(
		...
		long long GroupID,// 目标群 
		long long QQID,// 被设置的QQ 
		const char *NewGroupNickname// 
		);
	//置群退出 Auth=127 慎用,此接口需要严格授权 
	CQAPI(CQ_setGroupLeave, int)(
		int AuthCode,// 
		long long GroupID,// 目标群 
		CQBOOL Dismiss// 真/解散本群 (群主) 假/退出本群 (管理、群成员) 
		);
	//置讨论组退出 Auth=140 
