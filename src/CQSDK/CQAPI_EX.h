...
namespace CQ {
	...
	int setGroupKick(
		...
		CQBOOL Refuse_Request_Forever = false // 如果为真，则“不再接收此人加群申请”，请慎用 
	);

	//置群员禁言 Auth=121 
	int setGroupBan(
		long long GroupID, long long QQID,
		long long BanTime = 60 // 禁言的时间，单位为秒。如果要解禁，这里填写0 
	);

	//置群管理员 Auth=122 
	int setGroupAdmin(
		long long GroupID, long long QQID,
		CQBOOL Administrator = true // 真/设置管理员 假/取消管理员 
	);

	//置群成员专属头衔 Auth=128 需群主权限 
	int setGroupSpecialTitle(
		long long GroupID, long long QQID,
		const char *Rank, // 如果要删除，这里填空 
		long long = -1 // 专属头衔有效期，单位为秒。如果永久有效，这里填写-1
	);
	//置群成员专属头衔 Auth=128 需群主权限 
	int setGroupSpecialTitle(
		long long GroupID, long long QQID,
		std::string&Rank, // 如果要删除，这里填空 
	...
	int setGroupWholeBan(
		long long GroupID,
		CQBOOL WholeBan = true // 真/开启 假/关闭 
	);

	//置匿名群员禁言 Auth=124 
	int setGroupAnonymousBan(
		long long GroupID,
		const char *Anonymous, // 群消息事件收到的“匿名”参数 
		long long BanTime = 60 // 禁言的时间，单位为秒。不支持解禁 
	);

	//置群匿名设置 Auth=125 
	int setGroupAnonymous(long long GroupID, CQBOOL 开启匿名 = true);
	...
	int setGroupLeave(
		long long GroupID,
		CQBOOL Dismiss = false // 真/解散本群 (群主) 假/退出本群 (管理、群成员) 
	);
	...
	int setFriendAddRequest(
		const char *ResponseFlag, // 请求事件收到的“反馈标识”参数 
		int ResponseType, // #请求_通过 或 #请求_拒绝 
		const char *Remark // 添加后的好友备注 
	);
	...
	public:
		...
		int UniqueRankExpireTime; // -1 代表不过期
